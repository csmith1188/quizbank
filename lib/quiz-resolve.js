...
async function getQuestionsForUnit(unitId, courseId) {
    ...
    const rows = await all(
        `SELECT q.id, q.prompt, q.correct_answer, q.correct_index, q.answers FROM questions q JOIN tasks t ON q.task_id = t.id WHERE q.task_id IN (${placeholders}) AND t.course_id = ?`,
        [...ids, courseId]
    );
...
async function getQuestionsForTask(taskId, courseId) {
    const rows = await all(
        'SELECT q.id, q.prompt, q.correct_answer, q.correct_index, q.answers FROM questions q JOIN tasks t ON q.task_id = t.id WHERE q.task_id = ? AND t.course_id = ?',
        [taskId, courseId]
    );
    return rows.map(rowToQuestion);
}
...
async function getQuestionById(questionId, courseId) {
    const row = await get(
        'SELECT q.id, q.prompt, q.correct_answer, q.correct_index, q.answers FROM questions q JOIN tasks t ON q.task_id = t.id WHERE q.id = ? AND t.course_id = ?',
        [questionId, courseId]
    );
...
async function resolveQuizToQuestions(courseId, quizId) {
    const rows = await all(
        `SELECT q.id, q.prompt, q.correct_answer, q.correct_index, q.answers
...
/**
 * Pick questions from a source (unit or task) at add-time: all or random N.

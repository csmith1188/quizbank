...
const { getMasteryWindow } = require('./quiz-attempts');
...
async function pickProgressQuestions(userId, courseId, count) {
    ...
    for (let i = 0; i < eligibleTasks.length; i++) {
        ...
        const qs = await all(
            "SELECT id FROM questions WHERE task_id = ? AND COALESCE(quality, '') != 'bad' ORDER BY id",
            [t.id]
        );
...
async function createProgressAttempt(userId, classId, courseId) {
    // Only relaxed intensity shortens progress tests; standard and intense stay at 10.
    // Intense still uses a 20-answer mastery window when grading.
    const masteryWindow = await getMasteryWindow(classId, courseId);
    const questionCount =
        masteryWindow === config.masteryWindowRelaxed
            ? config.masteryWindowRelaxed
            : config.progressTestQuestionCount;
    const questionIds = await pickProgressQuestions(userId, courseId, questionCount);
...
// Pick up to `count` questions for an "overall knowledge" test, sampling
// randomly across all *units* in the course as evenly as possible,
// regardless of mastery.
async function pickOverallQuestions(courseId, count) {
    const desiredCount = count || config.overallTestQuestionCount;
    const rows = await all(
        `SELECT q.id as question_id, q.task_id, ut.unit_id
         FROM questions q
    ...
    // Group questions by unit (including null/"Ungrouped")
    const byUnit = new Map();
    rows.forEach(r => {
        const unitId = r.unit_id || 0; // 0 = Ungrouped
        ...
        byUnit.get(unitId).pool.push(r.question_id);
    });

    const units = Array.from(byUnit.values());
    if (!units.length) return [];

    // Shuffle question pools within each unit
    units.forEach(u => {
        for (let i = u.pool.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [u.pool[i], u.pool[j]] = [u.pool[j], u.pool[i]];
        }
    });

    // Round-robin pick across units to keep distribution roughly even
    const picked = [];
    let remaining = units.some(u => u.pool.length > 0);
    while (picked.length < desiredCount && remaining) {
        remaining = false;
        for (const u of units) {
            if (!u.pool.length) continue;
            picked.push(u.pool.pop());
            if (picked.length >= desiredCount) break;
            remaining = remaining || u.pool.length > 0;
        }
    }

    // Final shuffle so questions are not grouped strictly by unit
...
// Create an "overall knowledge" attempt that does NOT affect mastery.
async function createOverallAttempt(userId, classId, courseId) {
    const questionIds = await pickOverallQuestions(courseId, config.overallTestQuestionCount);
    if (!questionIds.length) {
        throw new Error('No questions available for overall knowledge test');
    }

    await run(
        'INSERT INTO quiz_attempts (user_id, class_id, quiz_id, total_questions, correct_questions, score, is_progress_quiz, metadata) VALUES (?, ?, ?, ?, 0, 0, 0, ?)',
        [userId, classId, 0, questionIds.length, JSON.stringify({ type: 'overall', courseId })]
    );
    const attempt = await get(
        'SELECT * FROM quiz_attempts WHERE user_id = ? AND class_id = ? AND is_progress_quiz = 0 AND json_extract(metadata, \'$.type\') = \'overall\' ORDER BY id DESC LIMIT 1',
        [userId, classId]
    );
    if (!attempt) {
        throw new Error('Failed to create overall knowledge attempt');
    }
...
module.exports = {
    ...
    pickOverallQuestions,
    createOverallAttempt
};

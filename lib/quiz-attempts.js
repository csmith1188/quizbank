...
async function getCoachContext(attemptId) {
    ...
    } else if (attempt.metadata) {
        // Non-progress attempts may still carry course metadata (e.g. overall knowledge tests)
        try {
            const meta = JSON.parse(attempt.metadata);
            if (meta && meta.courseId) {
                const row = await get('SELECT id, name FROM courses WHERE id = ?', [
...
const MASTERY_WINDOWS = {
    relaxed: config.masteryWindowRelaxed,
    standard: config.masteryWindowStandard,
    intense: config.masteryWindowIntense
};
const DEFAULT_MASTERY_WINDOW = MASTERY_WINDOWS.standard;

async function getMasteryWindow(classId, courseId) {
    if (!classId || !courseId) return DEFAULT_MASTERY_WINDOW;
    try {
        const row = await get(
            'SELECT mastery_intensity FROM class_courses WHERE class_id = ? AND course_id = ?',
            [classId, courseId]
        );
        const key = row && row.mastery_intensity ? String(row.mastery_intensity).toLowerCase() : 'standard';
        return MASTERY_WINDOWS[key] || DEFAULT_MASTERY_WINDOW;
    } catch (e) {
        return DEFAULT_MASTERY_WINDOW;
    }
}
 ...
 */
async function getLastNTaskMastery(userId, courseId, taskId, windowSize) {
    const rows = await all(
        `SELECT qaa.is_correct
         ...
         AND (
             ...
             OR (
                 qa.is_progress_quiz = 0
                 AND CAST(json_extract(qa.metadata, '$.courseId') AS INTEGER) = ?
                 AND json_extract(qa.metadata, '$.type') = 'overall'
             )
             OR (qa.is_progress_quiz = 1 AND CAST(json_extract(qa.metadata, '$.courseId') AS INTEGER) = ?)
         )
         ORDER BY qa.completed_at DESC, qaa.id DESC
         LIMIT ?`,
        [taskId, userId, courseId, courseId, courseId, windowSize]
    );
 ...
 */
async function cleanupOldTaskAnswers(userId, courseId, taskId, windowSize) {
    await run(
               ...
               AND old.id NOT IN (
                   ...
                   WHERE recent.task_id = ? AND qa2.user_id = ?
                     ...
                     AND (
                         ...
                         OR (
                             qa2.is_progress_quiz = 0
                             AND CAST(json_extract(qa2.metadata, '$.courseId') AS INTEGER) = ?
                             AND json_extract(qa2.metadata, '$.type') = 'overall'
                         )
...
async function gradeAttempt(attemptId) {
    ...
    if (attemptRow.is_progress_quiz) {
        ...
        // Non-progress attempts can still carry course metadata (e.g. overall tests)
        if (courseId == null && attemptRow.metadata) {
            try {
                const meta = JSON.parse(attemptRow.metadata);
                if (meta && meta.courseId) {
                    courseId = meta.courseId;
    ...
    if (courseId != null) {
        const masteryWindow = await getMasteryWindow(attemptRow.class_id, courseId);
        ...
        for (const [taskIdStr] of Object.entries(taskStats)) {
            ...
            await cleanupOldTaskAnswers(attemptRow.user_id, courseId, taskId, masteryWindow);
        }
    }

...
function convertToBlackboard(sectionId, unitId, taskId) {
    ...
    // Generate filename
    const filename = `blackboard_${section.name.replace(/\s+/g, '_')}_${unit.name.replace(/\s+/g, '_')}_${task.name.replace(/\s+/g, '_')}.dat`;
    const filepath = path.join(exportsDir, filename);
    
    // Write to file
    console.log(`Writing to ${filepath}...`);
    fs.writeFileSync(filepath, blackboardXML);
    
    // Create manifest file
    const manifestFilename = `imsmanifest_${section.name.replace(/\s+/g, '_')}_${unit.name.replace(/\s+/g, '_')}_${task.name.replace(/\s+/g, '_')}.xml`;
    const manifestFilepath = path.join(exportsDir, manifestFilename);
    console.log(`Writing manifest to ${manifestFilepath}...`);
    const manifestXML = createManifestXML(filename);
    fs.writeFileSync(manifestFilepath, manifestXML);
    
    console.log('\nConversion complete!');
    console.log('Files created in exports folder:');
    console.log(`- ${filename} (main test file)`);
    console.log(`- ${manifestFilename} (manifest file)`);
    console.log('');
